...
class ResponseBot(Plugin):
    ...
    if evt.sender in self.config["whitelist"]:
      ...
      if resp.status == 200:
        ...
        if "video" in mime_type:
          uri = await self.client.upload_media(data, mime_type=mime_type, filename=file_name)
       ...
       await evt.reply(f"Your mother is a hamster and your father smells of elderberries!")
  ...
  async def test(self, evt: MessageEvent) -> None:
    ...
    content = MediaMessageEventContent(url=url,
                                       body="Response.mp4",
